...
INSERT INTO `Room` (`RoomID`,`Room_Config`,`PC_NUM`) VALUES ('MPO310', 1, 41);
  /*──────────────────────── 3. Computers ────────────────────────*/
...
CREATE TABLE `Students` (
  `StudentNo`  CHAR(10)    NOT NULL COMMENT 'Format: YYYYxxxxxx (e.g., 2020135889)',
  `FirstName`  VARCHAR(50) NOT NULL,
  `LastName`   VARCHAR(50) NOT NULL,
  `ContactNo`  VARCHAR(30),
  `CreatedAt`  DATETIME    NOT NULL DEFAULT CURRENT_TIMESTAMP,
...
CREATE TABLE `BorrowedItems` (
  ...
  `ItemCategory`  ENUM('RJ45','Serial Cable','Keyboard','Mouse','PowerSupply','Other')
                    NOT NULL,
  `ItemDetails`   VARCHAR(100) NULL COMMENT 'Model, serial, or notes',
...
ALTER TABLE Fixes
  ADD COLUMN ServiceTicketID VARCHAR(100) NULL,
  ADD CONSTRAINT fk_fixes_ticket
        FOREIGN KEY (ServiceTicketID)
        REFERENCES ComputerStatusLog(ServiceTicketID)
        ON UPDATE CASCADE
        ON DELETE SET NULL;

CREATE TABLE IF NOT EXISTS TicketSeq (
...
BEGIN
  ...
  /* ② first issue (may be NULL) */
  SET issueText = SUBSTRING_INDEX(NEW.Issues, ',', 1);

  /* ③ fallback to “PM” when empty */
  IF issueText IS NULL OR issueText = '' THEN
      SET issueText = 'PM';
  END IF;

  /* ④ build ticket:  ROOM-PC-ISSUE-000000123 */
  SET NEW.ServiceTicketID = CONCAT(
        NEW.RoomID, '-', NEW.PCNumber, '-', issueText, '-', LPAD(nxt, 9, '0')
      );
END$$
